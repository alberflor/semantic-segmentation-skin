...
\begin{frame}
    \frametitle{Antecedentes}
    \begin{equation}
        M_t = \beta_1 M_{t-1}+(1-\beta_1)\nabla L_t(W_{t-1})   % Primer momento
    \end{equation}

    \begin{equation}
        R_t = \beta_2 R_{t-1}+(1-\beta_2)\nabla L_t(W_{t-1})^2   % Segundo momento
    \end{equation}

    \begin{equation}
        \hat{M}_t = M_t/\left(1-(\beta_1)^t\right)   % Correción del sesgo del primer momento
    \end{equation}
    
    \begin{equation}
        \hat{R}_t = R_t/\left(1-(\beta_2)^t\right)   % Corección del sesgo del segundo momento
    \end{equation}

    \begin{equation}
        W_t = W_{t-1}-\alpha\frac{\hat{M_t}}{\sqrt{\hat{R_t}+\epsilon}}   % Actualización
    \end{equation}
...
\begin{frame}
    ...
    \begin{figure}
        ...
        \subfloat[Histograma Original\label{fig:c}]{\hspace{-0.40cm}\includegraphics[scale=0.5]{../Plots/THR/threshold_input_0.png}} \qquad
        \subfloat[Histograma Resultante\label{fig:c}]{\hspace{.25cm}\includegraphics[scale=0.5]{../Plots/THR/threshold_output_0.png}}
