...
Para completar la fase de entrenamiento y poder continual a la fase de predicción, es necesario reducir las dimensiones de las imágenes y hacerlas pasar por algo denominado filtro de convolución.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw[step=1cm, black, thin](-2,-2) grid (5,5);
        \filldraw[fill = gray!20, draw = black] (0,0) rectangle (3,3);
        \draw[step=1cm, black, thin](0,0) grid (3,3);
    \end{tikzpicture}
    \caption{Matriz de pixeles de entrada.}
\end{figure}

\subsection{Modelo FPN}
La arquitectura de red neuronal tipo red piramidal de características, por sus siglas en inglés FPN\footnote{Feature Pyramid Network}, se trata de una estructura que funciona en conjunto con el codificador \texttt{Resnet}, conforme se van aplicando los filtros de convolución del codificador. 
