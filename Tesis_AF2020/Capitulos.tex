...
\coltext{Crear diagrama de vectores.}
...
Las imágenes por sí mismas pueden crear un modelo segmentación eficiente, sin embargo para que puedan ingresar al modelo es necesario que tenga una forma específica. En el caso del modelo de segmentación es necesario que la matriz de pixeles se encuentre en forma de \emph{tensor}, esto se puede hacer mediante el pre-procesado.

\coltext{Diagrama de pre-procesamiento de imagen a tensor.}

Si no se cuenta con una base de datos muy grande se puede aplicar transformaciones al set de entrenamiento, lo cuál genera más datos a partir del mismo set mediante la aplicación de transformaciones tales como rotaciones, espejeado, efectos de color aleatorios, entre otros.

\subsection{Entrenamiento del Modelo}
Antes de poder realizar segmentaciones con el sistema desarrollado, es necesario entrenarlo para poder segmentar el objeto deseado, esto es denominado \emph{aprendizaje supervisado} y se refiere a que el modelo hará predicciones basandose únicamente de los datos que le fueron provistos. 
