...
En la subsección de \emph{diseño experimental}, se explica en qué consiste y qué valor se pretende calcular para determinar los mejores parámetros en el ajuste del modelo de la red neuronal profunda.

En \emph{resultados}, se reportan los datos obtenidos en base al experimento llevado a cabo.

Y para finalizar \emph{discusión}, se llega a la conclusión final dependiendo de los experimentos llevados a cabo.


\section{Estadísticas de los Datos de Entrada} 
Para poder determinar la secuencia de transformaciones ideal para el conjunto de datos presente es necesario estudiar las estadísticas de las imágenes correspondientes, este experimento también es necesario para determinar el codificador ideal con la mejor resolución disponible en relación con la complejidad computacional.
\subsection{Diseño Experimental}
\subsection{Resultados}
\subsection{Discusión}

\section{Evaluación del Aprendizaje}
Una vez seleccionado el codificador ideal, es necesario evaluar el desempeño del aprendizaje mediante la comparación del mapa binario de la máscara real y el mapa probabilistico de la máscara obtenida por la configuración actual. Para esto se utiliza el coeficiente de dados (\emph{dice coefficient}), mediante el DC se puede evaluar la similitud entre dos matríces de mismas dimensiones, por lo tanto 
\begin{equation}
    \text{DC} = \frac{2|A \cap B |}{|A| + |B|}
\end{equation}
donde $A$ es un conjunto de intensidad de pixeles que representa la máscara real y $B$ la máscara estimada por la segmentación.
\subsection{Diseño Experimental}
\subsection{Resultados}
\subsection{Discusión}

\section{Validación de Máscaras}
Para finalizar la validación de la herramienta propuesta, se compararon las máscaras obtenidas mediante la computación del modelo con la respectiva máscara real. 
